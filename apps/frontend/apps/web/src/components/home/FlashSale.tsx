...
function getNextFlashSaleDeadline(): number {
  const now = new Date();
...
function FlashTile({ product }: { product: Product }) {
  const { productId, productTitle, priceUnit, imageUrl, quantity = 100 } = product;
  const sold = 0; // Mock sold count for now
  const stockTotal = quantity > 0 ? quantity : 100;
  const ratio = Math.min(100, Math.round((sold / stockTotal) * 100));
  const discountPercent = 10; // Mock discount
...
export default function FlashSale() {
  ...
  const [deadline] = useState(getNextFlashSaleDeadline);
  const [remaining, setRemaining] = useState(() => deadline - Date.now());

  const { data: flashSaleProducts = [] } = useProducts({ page: 0, size: 8 });
